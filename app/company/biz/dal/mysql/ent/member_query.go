...
func (mq *MemberQuery) loadMemberEntry(ctx context.Context, query *EntryLogsQuery, nodes []*Member, init func(*Member), assign func(*Member, *EntryLogs)) error {
	...
	for _, n := range neighbors {
		fk := n.member_member_entry
		if fk == nil {
			return fmt.Errorf(`foreign-key "member_member_entry" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "member_member_entry" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
