...
func (uq *UserQuery) loadUserEntry(ctx context.Context, query *EntryLogsQuery, nodes []*User, init func(*User), assign func(*User, *EntryLogs)) error {
	...
	for _, n := range neighbors {
		fk := n.user_user_entry
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_user_entry" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_user_entry" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
