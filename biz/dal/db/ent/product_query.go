...
func (pq *ProductQuery) QueryContracts() *ContractQuery {
...
func (pq *ProductQuery) QueryCourses() *ProductCoursesQuery {
	query := (&ProductCoursesClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProductQuery) QueryLessons() *ProductCoursesQuery {
	query := (&ProductCoursesClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (pq *ProductQuery) WithContracts(opts ...func(*ContractQuery)) *ProductQuery {
	query := (&ContractClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withContracts = query
	return pq
}
...
func (pq *ProductQuery) WithCourses(opts ...func(*ProductCoursesQuery)) *ProductQuery {
	query := (&ProductCoursesClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) WithLessons(opts ...func(*ProductCoursesQuery)) *ProductQuery {
	query := (&ProductCoursesClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withContracts; query != nil {
		if err := pq.loadContracts(ctx, query, nodes,
			func(n *Product) { n.Edges.Contracts = []*Contract{} },
			func(n *Product, e *Contract) { n.Edges.Contracts = append(n.Edges.Contracts, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withCourses; query != nil {
		if err := pq.loadCourses(ctx, query, nodes,
			func(n *Product) { n.Edges.Courses = []*ProductCourses{} },
			func(n *Product, e *ProductCourses) { n.Edges.Courses = append(n.Edges.Courses, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withLessons; query != nil {
...
func (pq *ProductQuery) loadContracts(ctx context.Context, query *ContractQuery, nodes []*Product, init func(*Product), assign func(*Product, *Contract)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Product)
	nids := make(map[int64]map[*Product]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		columns := s.SelectedColumns()
		s.Select(joinT.C(product.ContractsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
func (pq *ProductQuery) loadCourses(ctx context.Context, query *ProductCoursesQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductCourses)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(productcourses.FieldProductID)
	}
	query.Where(predicate.ProductCourses(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.CoursesColumn), fks...))
	}))
...
func (pq *ProductQuery) loadLessons(ctx context.Context, query *ProductCoursesQuery, nodes []*Product, init func(*Product), assign func(*Product, *ProductCourses)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Product)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.ProductCourses(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(product.LessonsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
